...
# File CSV untuk menyimpan permanen
...
if 'history' not in st.session_state:
    st.session_state.history = pd.DataFrame(columns=["Tanggal", "Tinggi (cm)", "Jenis Kelamin", "Berat Aktual", "Berat Ideal (kg)", "Status Gizi"])
...
# --------------------
# Tombol Hitung dan Simpan
# --------------------
if st.button("🔍 Hitung dan Simpan"):
    if tinggi > 0:
        dasar = tinggi - 100
        if jenis_kelamin == "Pria":
            berat_ideal = dasar - (dasar * 0.10)
        else:
            berat_ideal = dasar - (dasar * 0.15)
        ...
        <div style='text-align: center; font-size: 20px; background-color: #F3F4F6; border-radius: 10px'>
            <h2 style='color: #10B981;'>Berat Badan Ideal Anda: {berat_ideal:.if} kg</h2>
        ...
        st.warning("⚠️ Silakan masukkan tinggi badan yang valid (lebih dari 0 cm).")
...
# --------------------
#  Grafik Perkembangan Berat
# --------------------
st.subheader("📈 Grafik Perkembangan Berat Badan")
if not df.empty:
    fig = px.line(
        st.session_state.history.sort_values(by="Tanggal"),
        x="Tanggal",
        y="Berat Aktual",
